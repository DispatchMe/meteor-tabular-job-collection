...
TabularJobCollections = function (definitions) {
  _.each(definitions, function (definition, collectionName) {
    ...
    // On the client, create a helper so that the table object can be accessed by
    // {{TabularJobCollections.collectionName}}
    if (Meteor.isClient) {
...
TabularJobCollections._dateRenderFunction = function (val) {
  if (val instanceof Date) {
    // We have only a weak dependency on momentjs:moment, or moment may be defined globally in some other way
    if (typeof moment === 'function') return moment(val).format('llll');
    var momentjs = Package && Package['momentjs:moment'] && Package['momentjs:moment'].moment;
    if (momentjs) return momentjs(val).format('llll');
